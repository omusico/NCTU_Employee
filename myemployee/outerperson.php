...
$sqlOuterTitle="select * from OuterTitle order by ordersn desc";
				...
				if($.MyExtend.checkinput($(this).val())){
					...
					inputIdflag=false;
						...
						if($.MyExtend.checkid($(this).val())){
							...
							if(count>0){
									msg="<font color='red'>身分證字號重複</font>";
									inputIdflag=false;
								}
								else{
...
// var r=$("select[name='filetype[]'] option:checked").html();
// 	alert(r);
	...
	if($("input[name='nation']:checked").val()=="native" && $.MyExtend.CheckDomesticOrForeign()=="D"){
		if($("input[name='isStudent']:checked").val()=="y"){
			...
			for(var i=0;i<filetypearray.length;i++){
				...
				var grade_name=$("select[name='student_grade'] option:checked").html();
				if(grade_name=="博士候選人"){
					if(identity_flag && student_flag && phd_flag)
						ok_flag=true;
					else
						msg="需要[身分證/居留證][學生證][博士候選人證明]";
					}
				else{
					if(identity_flag && student_flag)
						ok_flag=true;
					else
						msg="需要[身分證/居留證][學生證]";
					}
		...
		if($("input[name='isStudent']:checked").val()=="y"){
				...
				alert(grade_name);
				if(grade_name=="博士候選人"){
					if(identity_flag && work_flag && student_flag && phd_flag)
						ok_flag=true;
					else
						msg="需要[身分證/居留證][學生證][工作證明][博士候選人證明]";
				}
				else{
					if(identity_flag && work_flag && student_flag)
						ok_flag=true;
					else
						msg="需要[身分證/居留證][學生證][工作證明]";
					}
	...
	<fieldset>
		...
		<form action="outerperson_add.php" name="apply_form" method="post"
			...
			本國籍<input type="radio" name="nation"
				...
				<tr>
					<td width="50%"><span id="nationid">身分證字號:</span></td>
