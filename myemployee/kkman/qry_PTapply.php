<?php
  ...
  if(strcmp($formAction,"applied")==0){//審核通過
	...
	//echo $strSQL;
	$result=$db->query($strSQL);
	echo "<script language='javascript'>alert('請核單 ".$OrderNo." 已取消審核狀態');</script>";
...
<input type="hidden" name="bugetno" id="bugetno" value="">
  <input name="radio" type="radio" id="radio" onClick="" value="1" checked>顯示兼任請核單
  <!--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		if($row=$result->fetch()){
			if($_SESSION["UserID"]==trim($row['CreateEmp']) || $_SESSION["power"]=="1"){
				...
				if($FormStatus=="-1"){//註銷不可列印單子
					...
					echo "<td><a href='' onClick=\"javascript:printForm('".trim($row['SerialNo'])."');\">兼任人員請核單</a></td>";
				}
