<?
	...
	//支領規則中OX表上的規則
	$result=Rules_noduplicate($OrderNo,$bugno,$PNo,$IdNo,$cstart,$cend,$Ptitle,$PayTypeStr,$Pay_unit,$FromEid);
	...
	//其他已請核身份從請核資料抓,不需要改成由支領資訊來抓,因為不管怎麼異動,使用的計畫都是同一個,加distinct就可以找出不同的計畫編號
	...
	$return_data['CheckedRules']="PS0";//記錄有跑過的規則,PS0=Rules_noduplicate
	foreach($condition as $temp){
		$bugType=checkBugetTypeForRules($bugno);//這次請核計畫是那一種類別
		if($temp[2]=="公務"){			
			if($bugType=="科技部" && $Ptitle!="4"){
				$return_data[$return_index++]=Rules_EmpAmountLimit_PS06($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$Ptitle,$cstart,$cend,$totalamount,$FromEid);
				$return_data['CheckedRules'].="_PS6";
			}else{
				$return_data[$return_index++]="公務人員不得兼任此類計畫";
			}
		}else if($temp[2]=="約用"){
			if($bugType=="教育部" && $Ptitle!="4"){
				$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
				$return_data[$return_index++]=Rules_EmpAmountLimit_PS05($OrderNo,$bugno,$PNo,$IdNo,$Prank,$payitem,$identity,$cstart,$cend,$totalamount,$FromEid);
				$return_data['CheckedRules'].="_PS4_PS5";
			}else if($bugType=="教育部" && $Ptitle=="4"){
				$return_data[$return_index++]=Rules_EmpAmountLimit_PS05($OrderNo,$bugno,$PNo,$IdNo,$Prank,$payitem,$identity,$cstart,$cend,$totalamount,$FromEid);
				$return_data['CheckedRules'].="_PS5";
			...
			if($temp[1]=="教育部"){
				if($bugType=="教育部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_EduAndOthers_AmountLimit_PS01($OrderNo,$PNo,$IdNo,$identity,$bugno,$cstart,$cend,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_EmpAmountLimit_PS05($OrderNo,$bugno,$PNo,$IdNo,$Prank,$payitem,$identity,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS1_PS4_PS5";
				}else if($bugType=="教育部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_EduAndOthers_AmountLimit_PS01($OrderNo,$PNo,$IdNo,$identity,$bugno,$cstart,$cend,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_EmpAmountLimit_PS05($OrderNo,$bugno,$PNo,$IdNo,$Prank,$payitem,$identity,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS1_PS5";
				}else if($bugType=="其他"){//其他類兼任人員和臨時工規則一樣
					$return_data[$return_index++]=Rules_EduAndOthers_AmountLimit_PS01($OrderNo,$PNo,$IdNo,$identity,$bugno,$cstart,$cend,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_EmpAmountLimit_PS05($OrderNo,$bugno,$PNo,$IdNo,$Prank,$payitem,$identity,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS1_PS5";
				...
				if($bugType=="教育部" && $Ptitle!="4"){
					...
					$return_data[$return_index++]="頂尖計畫專任人員不得兼任此類計畫";
				}
			}else if($temp[1]=="其他"){
				if($bugType=="教育部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
			...
			if($temp[1]=="教育部" && $temp[3]!="4"){
				if($bugType=="教育部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_StuLimit_PS09($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS4_PS9";
				}else if($bugType=="教育部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="頂尖" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_EmpLimit_PS08($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$Ptitle,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS8";
				}else if($bugType=="頂尖" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="科技部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_EduStu_AmountLimit_PS02($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					//20150306校外人士選科技部助教級/講師級助教
					$return_data[$return_index++]=Rules_EmpAmountLimit_PS06($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$Ptitle,$cstart,$cend,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_StuLimit_PS09($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS2_PS6_PS9";
				}else if($bugType=="科技部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="其他" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_Stu_SingleApplyLimit_PS03($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_StuLimit_PS09($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS3_PS9";
				}else if($bugType=="其他" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else{
				...
				if($bugType=="教育部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS4";
				}else if($bugType=="教育部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="頂尖" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_Stu_SingleApplyLimit_PS03($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS3";
				}else if($bugType=="頂尖" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="科技部" && $Ptitle!="4"){
					...
					//20150306校外人士選科技部助教級/講師級助教
					$return_data[$return_index++]=Rules_EmpAmountLimit_PS06($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$Ptitle,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS2_PS6";
				}else if($bugType=="科技部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="其他" && $Ptitle!="4"){
				...
				if($bugType=="教育部" && $Ptitle!="4"){
					...
					$return_data[$return_index++]=Rules_EmpAmountLimit_PS06($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$Ptitle,$cstart,$cend,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_EmpLimit_PS08($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$Ptitle,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS6_PS8";
				}else if($bugType=="科技部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="其他" && $Ptitle!="4"){
					...
					$return_data[$return_index++]="頂尖計畫兼任人員不得兼任此類計畫";
				}
			}else if($temp[1]=="頂尖" && $temp[3]=="4"){
				if($bugType=="教育部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					...
					$return_data[$return_index++]="頂尖計畫臨時工不得兼任此類計畫";
				}
				...
				if($bugType=="教育部" && $Ptitle!="4"){
					...
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_StuLimit_PS09($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS2_PS4_PS9";
				}else if($bugType=="教育部" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else if($bugType=="頂尖" && $Ptitle!="4"){
					...
					$return_data[$return_index++]=Rules_EduStu_AmountLimit_PS02($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					$return_data[$return_index++]=Rules_StuLimit_PS09($OrderNo,$bugno,$PNo,$IdNo,$identity,$Prank,$cstart,$cend,$totalamount,$FromEid);
					$return_data['CheckedRules'].="_PS2_PS9";
				}else if($bugType=="其他" && $Ptitle=="4"){
					$return_data[$return_index++]=Rules_StuLimit_PS07($OrderNo,$bugno,$PNo,$IdNo,$identity,$Ptitle,$PayTypeStr,$Pay_limit,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d,$FromEid);
					$return_data['CheckedRules'].="_PS7";
				}else{
				...
				if($bugType=="教育部" && $Ptitle!="4"){
					...
					$return_data[$return_index++]="科技部計畫臨時工不得兼任此類計畫";
				}
				...
				if($bugType=="教育部" && $Ptitle!="4"){
					...
					$return_data[$return_index++]="其他類計畫兼任人員不得兼任此類計畫";
				}
			}else if($temp[1]=="其他" && $temp[3]=="4"){
				if($bugType=="教育部" && $Ptitle!="4"){
					$return_data[$return_index++]=Rules_SingleApplyLimit_PS04($OrderNo,$bugno,$identity,$Prank,$totalamount,$FromEid);
					...
					$return_data[$return_index++]="其他類計畫臨時工不得兼任此類計畫";
				}
			}
		}
	}
	...
	//$jsonString = $j->encode(print_r($condition)); 
	echo $jsonString;
	exit;
?>
