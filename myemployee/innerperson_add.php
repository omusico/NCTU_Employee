...
if (isset ( $_POST ['action'] ) && $_POST ['action'] == "save") {
	...
	$sqlInsert="insert into OuterStatus (SerialNo,IdNo,IdCode,Name,Birthday,isNCTU,RecordStatus,isStudent,isJob,schoolName,comName,Education,studentGrade,UpdateDate,UpdateEmpNo,CreateDate,CreateEmp)";
	$sqlInsert.=" values('0000','".$id."','".$idcode."','".$name."','".$birth."','1','0','".$isStudent."','".$isJob."','".$schoolName."','".$comName."','".$job_grade."','".$student_grade."',getdate(),'".$empid."',getdate(),'".$empid."')";
	echo $sqlInsert;
	$db->exec($sqlInsert);
	...
	foreach ( $array as $key => $value ) { //循環讀取陣列中資料
		...
		if($filetype==4){
			...
			$sqlInsertPeriod.= " values('".$maxfid."','".$v_begin."','".$v_end."','0','".$empid."',getdate())";
	...
	$savetype=$_POST ['savetype'];
 			...
 			if($savetype=="save")
 				...
 				header("location:innerperson.php");
 			}
