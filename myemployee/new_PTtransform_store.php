...
<?php 
	...
	if($act=="newTransform"){//新增異動單
		...
		$strSQL="insert into PT_Outline ".
				"select '異動','".$worktype."','".$bugetno."','".$today."','".$_SESSION['UserID']."',null,null,'".$_SESSION['Dept']."','".$today."','".$_SESSION['UserID']."','0',null,null,v.*,null,v2.Name,d.主管姓名 from [PERSONDBOLD].[personnelcommon].[dbo].[vi_buget] v ".
		...
		foreach($selectedSN as $p){
			...
			$uploadErrMsg="";
			$uploadErrMsg2="";
			for($i=1;$i<=$num_rows;$i++){//取得有被異動的聘期
...
?>
<body bgcolor='#c1cfb4'>
	<form name="addPT" id="addPT" method="POST" action="new_PTtransform_store.php" target="_self" enctype="multipart/form-data">
	...
	<?
					...
					while($row=$result->fetch()){
						...
						echo "<tr id='tr_".$row['Eid']."_0'><td colspan='11'>異動後".
							 "&nbsp;&nbsp;<img src='pics/plus.gif' style='cursor:pointer;' onClick=\"javascript:add_Row('".trim($row['Eid'])."','".$bug_type."','".$row['PTtitle']."','".$row['start_y']."','".$row['start_m']."','".$row['start_d']."','".$row['end_y']."','".$row['end_m']."','".$row['end_d']."','".$paytype."','".$pay_unit."','".$pay_limit."','".$pay_total."','1');\">".
						...
						echo "上傳文件".
							 ...
							 "<input type='hidden' name='uploadrowcount_".trim($row['Eid'])."' id='uploadrowcount_".trim($row['Eid'])."' value='0'></div></td></tr>";
					}
	...
	?>
				...
				<input type="button" id="send_data" value="建立異動單" onClick="javascript:document.getElementById('send_data').disabled=true;next();document.getElementById('send_data').disabled=false;">
	<?		}else{echo "查無可異動資料";}
		}
	?>
	</form>
...
//TransType=1(預設,刪除聘期),2(補足破月用)
function add_Row(eid,bug_type,PTtitle,start_y,start_m,start_d,end_y,end_m,end_d,paytype,pay_unit,pay_limit,paytotalamount,TransType){
	var num=document.getElementById('rowcount_'+eid).value;
	...
	if(TransType=="1"){
		str+="<option value='1' selected>聘期刪除</option><option value='2'>金額變更</option></select></td>";
		for(i=parseInt(start_y,10);i<=parseInt(end_y,10);i++){
			yearStr+="<option value='"+i+"'";
			if(i==parseInt(start_y,10)){yearStr+=" selected";}
			yearStr+=">"+i+"</option>";
			yearStr2+="<option value='"+i+"'";
			if(i==parseInt(end_y,10)){yearStr2+=" selected";}
			yearStr2+=">"+i+"</option>";
		}
		if(start_y!=end_y){
			var mstart=1;
		...
		for(i=mstart;i<=mlimit;i++){
			...
			if(i==parseInt(start_m,10)){monthStr+=" selected";}
			monthStr+=">"+i+"</option>";
			monthStr2+="<option value='"+i+"'";
			if(i==parseInt(end_m,10)){monthStr2+=" selected";}
			monthStr2+=">"+i+"</option>";
		}
		for(i=1;i<=31;i++){
			dayStr+="<option value='"+i+"'";
			if(i==parseInt(start_d,10)){dayStr+=" selected";}
			dayStr+=">"+i+"</option>";
			dayStr2+="<option value='"+i+"'";
			if(i==parseInt(end_d,10)){dayStr2+=" selected";}
			dayStr2+=">"+i+"</option>";
		}
	}
	if(TransType=="2"){
	...
	str +="<select name='paytype_"+eid+"_"+num+"' id='paytype_"+eid+"_"+num+"' onChange=\"javascript:ChangePayType('paytype_"+eid+"_"+num+"','pay_unit_"+eid+"_"+num+"','pay_limit_"+eid+"_"+num+"','totalamount_"+eid+"_"+num+"');ifShowUpload("+eid+");checkIfModification("+eid+","+num+");\"";
	if(TransType=="1"){str+=" disabled";}
	str +=">";
	...
	str += "&nbsp;&nbsp;";
	str += "<input type='text' name='pay_unit_"+eid+"_"+num+"' id='pay_unit_"+eid+"_"+num+"' value='"+pay_unit+"' size='5' onChange=\"javascript:checkAmount('paytype_"+eid+"_"+num+"','pay_unit_"+eid+"_"+num+"','pay_limit_"+eid+"_"+num+"','totalamount_"+eid+"_"+num+"');checkIfModification("+eid+","+num+");\" readonly disabled>單位/元&nbsp;&nbsp;&nbsp;&nbsp;";
	str += "每月上限<input type='text' name='pay_limit_"+eid+"_"+num+"' id='pay_limit_"+eid+"_"+num+"' value='"+pay_limit+"' size='5' onChange=\"javascript:checkAmount('paytype_"+eid+"_"+num+"','pay_unit_"+eid+"_"+num+"','pay_limit_"+eid+"_"+num+"','totalamount_"+eid+"_"+num+"');checkIfModification("+eid+","+num+");\" readonly disabled> 單位";
	str += "&nbsp;&nbsp;&nbsp;&nbsp;總額：<input type='text' name='totalamount_"+eid+"_"+num+"' id='totalamount_"+eid+"_"+num+"' value='"+paytotalamount+"' size='5' readonly disabled></td>";
	...
	for(i=0;i<DelayCode.length;i++){
		str += "<option value='"+DelayCode[i]+"'>"+DelayName[i]+"</option>";
	}
	str += "</select><br><input type='text' name='DelayReason_"+eid+"_"+num+"' id='DelayReason_"+eid+"_"+num+"' size='30' value=''></td>";
	str += "<td>備註:<input type='text' name='comment_"+eid+"_"+num+"' size='20'>";
	str += "</td>";
	str	+="</tr>";
	...
	if(ErrMsg_All!=""){
		...
		if(ErrMsg){
			...
			for(index=0;index<selectedSN.length;index++){
				...
				var Mod_ErrMsg=$.trim(Mod_ErrMsg);
			...
			if(ErrMsg_All!=""){alert(ErrMsg_All);}
				...
				//20150310要求取消confirm
				//if(confirm("確定送出??")){
					//確定送出時,將聘期刪除部分的支領方式select tag undisabed,否則會抓不到
					for(index=0;index<selectedSN.length;index++){
						var rowcount=document.getElementById('rowcount_'+selectedSN[index]).value;
						if(rowcount>0){
							for(i=1;i<=parseInt(rowcount,10);i++){
		...
		if(ErrMsg!=""){
			...
			if(confirm(ErrMsg2)){
				for(i=0;i<needAddEid.length;i++){
					...
					add_Row(needAddEid[i],"<?echo $bug_type;?>",PTtitle,yy,mm,needAdd_start[i],yy,mm,needAdd_end[i],paytype,pay_unit,pay_limit,paytotalamount,"2","1");					
				}
				alert("新增完成");
				return false;
			}
