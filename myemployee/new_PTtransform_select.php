...
<body bgcolor='#c1cfb4'>
	<form name="addPT" id="addPT" method="POST" action="new_PTtransform_select.php" target="_self">	
		...
		<table width="720" border="1" cellpadding="0" cellspacing="0" bordercolor="#111111" id="SciPayment" style="border-collapse: collapse">
			...
			<tr>
			  <td bgcolor="#DFDFDF">
					<?	if($_SESSION["power"]=="1"){ ?>
						<input type="text" name="bugetno" id="bugetno" value="<?echo $bugetno;?>">
					<?  }else{	
							$str="select * from officer_project where UserAccount='".$_SESSION['UserID']."'";
							$result=$db->query($str);
							$row=$result->fetchAll();
							$num=count($row);
							if($num>0){
								echo "<select name='bugetno' id='bugetno'>".
									 "<option value=''>&nbsp;</option>";
								$result=$db->query($str);
								while($row=$result->fetch()){
									echo "<option value='".trim($row['BugetNo'])."'";
									if(trim($row['BugetNo'])==$bugetno){echo " selected";}
									echo ">".$row['BugetNo']."</option>";
								}
								echo "</select>";
							}else{echo "無法請核，請先向計畫主持人申請計畫承辦人權限";}
						}
						if($_SESSION["power"]=="1" || $num>0){?>
						<input type="button" value="查詢" onclick="checkBugetno();">
					<? }?>
			  </td>
			</tr>
		</table>
			...
			if(count($row)>0){	?>
				<fieldset border: solid 10px blue;>
					...
					<table width="900"  cellspacing="1" cellpadding="4">		
						<tr height="20" align="left" bgcolor="#C9CBE0">
							<td>選取<br><input type="button" value="全選" onClick="javascript:selectall();"></td>
							<td>單號</td>
							<td>人員編號</td>		 		
							<td>姓名</td>
							<td>兼任職稱</td>
	...
	<?
						...
						while($row=$result->fetch()){
							...
							$strSQL="select distinct SerialNo from PT_Employed where FromEid='".$row['Eid']."' and RecordStatus in ('0','-2') and SerialNo in (select SerialNo from PT_Outline where FormStatus='0' and CreateEmp='".$_SESSION[UserID]."')";
							//echo $strSQL;
