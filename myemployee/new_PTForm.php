	...
	$(function(){
		...
		$("input[name='editfile']").click(function(){
			...
			$("#removeFile").click(function() {
				$("#clonediv:last").remove();
				//alert($("#clonediv").size);
			});
			
			$("#introFile").click(function(){
...
<?php 
	...
	if(isset($_POST['noWorkIDcheck_byPrjLeader'])){
		$noWorkIDcheck_byPrjLeader="1";
	}else{
		$noWorkIDcheck_byPrjLeader="0";
	}
	$aborigin=$_POST['aborigin'];
	if($aborigin==""){$aborigin="0";}
	$disability=$_POST['disability'];
	if($disability==""){$disability="0";}
	...
	$DelayCode=$_POST['TraceBackReasonCode'];
	...
	if($formAction=="newPTapply"){//新增請核單		
		...
		$strSQL="insert into PT_Outline ".
				"select '請核','".$worktype."','".$bugetno."','".$today."','".$_SESSION['UserID']."',null,null,'".$_SESSION['Dept']."','".$today."','".$_SESSION['UserID']."','0',null,null,v.*,null,v2.Name,d.主管姓名 from [PERSONDBOLD].[personnelcommon].[dbo].[vi_buget] v ".
				"left join [PERSONDBOLD].[約用人員資料庫].[dbo].[DepartmentCode] d on d.code=v.leaderid ".
				"left join [PROJECT].[Plan].[dbo].[View_Plan_Detail_PersonDB] v2 on (v.bugetno collate Chinese_Taiwan_Stroke_CI_AS)=(v2.School_Num collate Chinese_Taiwan_Stroke_CI_AS) where v.bugetno='".$bugetno."'";
		$result=$db->query($strSQL) or die($strSQL);
		$strSQL="select * from PT_Outline where updatedate='".$today."'";
		$result=$db->query($strSQL);
		$row=$result->fetch();
		$OrderNo=$row['SerialNo'];
	}else if($formAction=="add" && $FormStatus!="-2"){//新增兼任人員
		//加入請核單明細中
		$strSQL="insert into PT_Employed select '".$OrderNo."','".$PNo."','".$IdNo."','".$Pname."','".$Prank."','".$PT_Identity."','".$Ptitle."','".$worktype."','".$noWorkIDcheck_byPrjLeader."','".($start_y+1911)."-".$start_m."-".$start_d."','".($end_y+1911)."-".$end_m."-".$end_d."','".$payitem."',null,null";
		if($paytype=="hr_pay" || $paytype=="day_pay" || $paytype=="case_pay"){
			...
			if($paytype=="month_pay"){$strSQL.=",'".($pay_unit*$pay_limit)."',null,null,'".$totalpay."'";}
			else{$strSQL.=",null,'".$pay_unit."','".$pay_limit."','".$totalpay."'";}
		}
		$strSQL.=",'".$DelayCode."','".$TraceBackReason."','".$comment."','".$aborigin."','".$disability."','".$relatives."','".$today."','".$_SESSION['UserID']."','".$today."','".$_SESSION['UserID']."',null,null,null,'0',null,null,null,null";
		//echo $strSQL."<br>";
		...
		//給pt_employed記錄補上FirstEid
		$strSQL="update PT_Employed set FirstEid='".$Eid."' where Eid='".$Eid."'";
		$db->query($strSQL);
		//處理上傳Begin
		//================================================
		...
		foreach ( $array as $key => $value ) { //循環讀取陣列中資料
			...
			$dot_position=strrpos($_FILES ["upload"] ["name"] [$key],".");
			...
			$arrayindex++;
			$status=2;//B類不用審核,預設為2
			//echo $_FILES ["upload"] ["name"] [$key]."<br/>";
			//echo "value=".$value."<br/>";
			//echo "key=".$key."<br/>";
			//echo "name=".$name."<br/>";
			//echo "filesize=".floatval($_FILES["upload"]["size"][$key]/(1024*1024))."MB<br/>";
			//echo "type=".$type."<br/>";
			
			if(floatval($_FILES["upload"]["size"][$key]/(1024*1024))<=2){
		...
		if($uploadErrmg!=""){$uploadErrmg="上傳檔案過大(超過2MB),未成功:\\n".$uploadErrmg;}
		$msg="新增兼任人員成功!!".$uploadErrmg;	
	}else if($formAction=="modify" && $FormStatus!="-2"){//修改兼任人員
		//修改請核單明細中,可以更換人員
		$strSQL="update PT_Employed set IdCode='".$PNo."',Pid='".$IdNo."',[Name]='".$Pname."',Title='".$Prank."',Role='".$PT_Identity."',PTtitle='".$Ptitle."',JobType='".$worktype."',noWorkIDcheck_byPrjLeader='".$noWorkIDcheck_byPrjLeader."',BeginDate='".($start_y+1911)."-".$start_m."-".$start_d."',EndDate='".($end_y+1911)."-".$end_m."-".$end_d."',JobItemCode='".$payitem."'";
		if($paytype=="hr_pay" || $paytype=="day_pay" || $paytype=="case_pay"){
			...
			if($paytype=="month_pay"){$strSQL.=",PayType=Null,PayPerUnit=Null,LimitPerMonth=Null,MonthExpense='".($pay_unit*$pay_limit)."',AwardUnit=Null,AwardLimit=Null,TotalAmount='".$totalpay."'";}
		...
		$strSQL.=",DelayCode='".$DelayCode."',TraceBackReason='".$TraceBackReason."',Memo='".$comment."',IsAboriginal='".$aborigin."',IsDisability='".$disability."',BossRelation='".$relatives."',UpdateDate='".$today."',UpdateEmp='".$_SESSION['UserID']."' where Eid='".$Eid."'";
		//echo $strSQL."<br>";
		$result=$db->query($strSQL);
		//刪除之前記的EmpStatus和StuStatus資料
		$strSQL="delete from EmpStatus where Eid='".$Eid."'";
		$strSQL2="delete from StuStatus where Eid='".$Eid."'";
		...
		//=============================================		
		//只有要上傳的狀況才處理上傳文件
		if(($Ptitle=="4" || $Ptitle=="15") && ($paytype=="day_pay" || $paytype=="month_pay" || $paytype=="case_pay")){
			$uploadErrmg="";
			$filetypes=$_POST ['filetype'];
			$filetitles=$_POST ['filetitle'];
		...
		}else{//若是更新為不需上傳的狀況,刪除已上傳文件,以免審核時出現
			$str="update UploadData set status='-1',updatedate='".$today."',UpdateEmp='".$_SESSION['UserID']."' where SEid='".$Eid."' and status='2'";
			$db->query($str);
		}
		//上傳處理End
		//=============================================
		...
		$noWorkIDcheck_byPrjLeader=trim($row['noWorkIDcheck_byPrjLeader']);
	...
	<fieldset>
			...
			<?	
				...
				if($OrderNo!=""){//已有新增人員,列出清單
					$strSQL="select pt.*,".
							...
							" from PT_Employed pt ".							
							"where pt.SerialNo='".$OrderNo."' and pt.RecordStatus in ('0','-2') order by eid desc";
					//echo $strSQL;
					$result=$db->query($strSQL);
					while($row=$result->fetch()){
						...
						echo "<td>".$row['Memo']."</td>";
						if($row['RecordStatus']=="-2"){echo "<td>已異動</td>";}
						else{
							echo "<td><div id='func_div_".trim($row['Eid'])."'>";
							if($formAction=="loading" && $Eid==trim($row['Eid'])){echo "<strong>修改中...</strong>";}
							else{
								echo "<input type='button' value='修改' onClick='javascript:loadingPT(".$OrderNo.",".trim($row['Eid']).");'>";
								echo "<input type='button' value='刪除' onClick='javascript:deletePT(".$OrderNo.",".trim($row['Eid']).");'>";
							}
							echo "</div></td>";
						}
						echo "</tr>";
		...
		<table width="1000" border="0">				
			<tr height="20" align="left">
				<td align="right"><input type="button" name="printform" id="printform" value="產出請核單/契約書" onClick="javascript:window.open(' https://Receipt-test.nctu.edu.tw/test/phpio_new.php?OrderNo=<?echo $OrderNo;?>&action=PTform1', '列印表單');"></td>
			</tr>							
		</table>
	</fieldset>
	<hr>
	...
	<fieldset>
		...
		<table width="1000"  cellspacing="1" cellpadding="4" bgcolor="#9194BF">				
			...
			<tr>
				...
				<td nowrap rowspan="4" width="120" bgcolor="#C9CBE0">支領類別</td>
				<td rowspan="4" bgcolor="FFFFCC" align="left"><!--底下輸入預設都無法使用,等上面填的差不多再開放-->					
					...
					<?if($is_hr=="0"){echo " readonly";}?>>元/時<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					每月時數上限<input type="text" name="hr_pay_limit" id="hr_pay_limit" value="<?if($formAction=="loading" && $paytype=="hr_pay"){echo $pay_limit;}?>" size="5" onChange="javascript:checkPayType('hr_pay','hr_pay_unit','hr_pay_limit');" <?if($is_hr=="0"){echo " readonly";}?>>小時<br>
					...
					<?
						...
						if($is_case=="0"){echo " disabled";}
					?>
					>
					按件計酬：
					<input type="text" name="case_pay_unit" id="case_pay_unit" value="<?if($formAction=="loading" && $paytype=="case_pay"){echo $pay_unit;}?>" size="5" onChange="javascript:checkPayType('case_pay','case_pay_unit','case_pay_limit');"  <?if($is_case=="0"){echo " readonly";}?>>元/件，<br>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					每月件數上限<input type="text" name="case_pay_limit" id="case_pay_limit" value="<?if($formAction=="loading" && $paytype=="case_pay"){echo $pay_limit;}?>" size="5" onChange="javascript:checkPayType('case_pay','case_pay_unit','case_pay_limit');" <?if($is_case=="0"){echo " readonly";}?>>件<br>
					...
					<?
						...
						if($is_day=="0"){echo " disabled";}
					?>
					>
					日&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;薪：
					<input type="text" name="day_pay_unit" id="day_pay_unit" value="<?if($formAction=="loading" && $paytype=="day_pay"){echo $pay_unit;}?>" size="5" onChange="javascript:checkPayType('day_pay','day_pay_unit','day_pay_limit');" <?if($is_day=="0"){echo " readonly";}?>>元/日<br>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					每月日數上限<input type="text" name="day_pay_limit" id="day_pay_limit" value="<?if($formAction=="loading" && $paytype=="day_pay"){echo $pay_limit;}?>" size="5" onChange="javascript:checkPayType('day_pay','day_pay_unit','day_pay_limit');" <?if($is_day=="0"){echo " readonly";}?>>日<br>
					...
					<?
						...
						if($is_award=="0"){echo " disabled";}
					?>
					>
					獎助單元：2000 &nbsp;
					<input type="hidden" name="award_pay_unit" id="award_pay_unit" value="2000" size="5" onChange="javascript:checkPayType('award_pay','award_pay_unit','award_pay_limit');" readonly>元/單元&nbsp;&nbsp;&nbsp;&nbsp;<br>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					每&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="award_pay_limit" id="award_pay_limit" value="<?if($formAction=="loading" && $AwardUnit!=""){echo $AwardLimit;}?>" size="5" onChange="javascript:checkPayType('award_pay','award_pay_unit','award_pay_limit');" <?if($is_award=="0"){echo " readonly";}?>>&nbsp;&nbsp;單元<br>
					...
					<?
						...
						if($is_month=="0"){echo " disabled";}
					?>
					>月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;薪：<input type="text" name="month_pay_unit" id="month_pay_unit" value="<?if($formAction=="loading" && $monthExpense!=""){echo $monthExpense;}?>" size="5" onChange="javascript:checkPayType('month_pay','month_pay_unit','month_pay_limit');" <?if($is_month=="0"){echo " readonly";}?>>元/月<br>
				...
				<td rowspan="4" colspan="2" bgcolor="#C9CBE0">
					<?if($formAction=="loading" && checkARC($IdNo)){?>
					<div id="Div_WorkingPeriod" style="">
					<?}else{?>
					<div id="Div_WorkingPeriod" style="display: none;">
					<?}?>
						<!--<input type="checkbox" name="noWorkIDcheck_byRelative" id="noWorkIDcheck_byRelative"><b>依親居留免辦理工作許可</b><br>-->
						<input type="checkbox" name="noWorkIDcheck_byPrjLeader" id="noWorkIDcheck_byPrjLeader"
						<?
							if($formAction=="loading" && $noWorkIDcheck_byPrjLeader=="1"){echo " checked";}
						?>
						><b>計畫主持人認定屬課程學習範疇之研究助理或教學助理，免申請工作許可</b>
						<br><br>
					</div>
					追溯請核理由:<br>
					<select name="TraceBackReasonCode" id="TraceBackReasonCode">
						<option value="">&nbsp;</option>
					<?
						$str_reason="select * from DelayType";
						$result_reason=$db->query($str_reason);
						while($row_reason=$result_reason->fetch()){
							echo "<option value='".$row_reason['DelayCode']."'";
							if($formAction=="loading" && $DelayCode==$row_reason['DelayCode']){echo " selected";}
							echo ">".trim($row_reason['DelayReason'])."</option>";
						}
					?>
					</select>
					<br><textarea name="TraceBackReason" id="TraceBackReason" rows="3" cols="25"><?if($formAction=="loading"){echo $TraceBackReason;}?></textarea><br>
			...
			<tr>
				<td bgcolor="#C9CBE0">原住民</td>
				<td bgcolor="FFFFCC">
					<input type="radio" name="aborigin" id="aborigin1" value="1" <?if($formAction=="loading" && $aborigin=="1"){echo  "checked";}?>>是&nbsp;
					<input type="radio" name="aborigin" id="aborigin2" value="0" <?if($formAction=="loading" && $aborigin=="0"){echo  "checked";}?>>否
				</td>
			</tr>
			<tr>
				<td bgcolor="#C9CBE0">身障人士</td>
				<td bgcolor="FFFFCC" >
					<input type="radio" name="disability" id="disability1" value="1" <?if($formAction=="loading" && $disability=="1"){echo  "checked";}?>>是&nbsp;
					<input type="radio" name="disability" id="disability2" value="0" <?if($formAction=="loading" && $disability=="0"){echo  "checked";}?>>否
				</td>
			</tr>
			<tr>
				<td bgcolor="#C9CBE0">直屬主管之配偶或三等內親眷</td>
				<td bgcolor="FFFFCC" >
					<?
						//20150316助理測試回饋,取消預設
						//if($formAction!="loading" || $relatives!="0"){echo  "checked";}
					?>
					<input type="radio" name="relatives" id="relatives1" value="1" <?if($formAction=="loading" && $relatives=="1"){echo  "checked";}?>>是&nbsp;
					<input type="radio" name="relatives" id="relatives2" value="0" <?if($formAction=="loading" && $relatives=="0"){echo  "checked";}?>>否
				</td>
			</tr>
			<!--<tr>
				<td colspan="6">
			...
			<?php if($formAction=="loading"){?>
			<?php 	if(($Ptitle=="4" || $Ptitle=="15") && ($paytype=="day_pay" || $paytype=="month_pay" || $paytype=="case_pay")){?>	
			...
			<?		}?>
					<fieldset>
						...
						<div id="upload_div">
						<?php 
							$sqlUploadFile="select * from UploadData where SEid='".$Eid."' and status<>'-1'";
							//echo $sqlUploadFile;
						...
						<input type="button" value="新增上傳欄位M" id="addFileModify">
						<input type="button" value="移除上傳欄位" id="removeFile">
						<input type="button" value="上傳說明" id="introFile">						
					</fieldset>
				</div>
			<?	}else{?>
				<div id="Div_upload" style="display: none;">
					<fieldset>
						...
						<input type="button" value="新增上傳欄位N" id="addFile">
						<input type="button" value="移除上傳欄位" id="removeFile">
						<input type="button" value="上傳說明" id="introFile">
					</fieldset>
				</div>
...
<?if($OrderNo!="" && $formAction!="loading"){?>
	...
	document.getElementById('getNormalPaycont').src='getBugno.php?bugno=<?echo $bugetno;?>&type=<?echo $worktype;?>&OrderNo=<?echo $OrderNo;?>';
<?}?>
<?if($formAction=="loading"){?>
...
function addUser(){	
	...
	if(identity=="O"){//校外人士要輸入姓名
		...
		name = $.trim(name);
		if(name==""){alert("校外人士請記得輸入姓名!!");return false;}
	}
	...
	if(PayType=="none" || PayType==""){alert("請選擇支領類別,和相對應的單位薪資和上限數字!");return false;}
		...
		if(parseInt(PayTotal)==0){alert("支領總金額不得為0");return false;}
	}
	//alert(document.addPT.relatives.value);
	if(document.addPT.relatives.value==""){alert("請選擇請核人是否為直屬主管之配偶或三等親等內之血親、姻親");return false;}
	if(document.addPT.relatives.value=="1"){alert("請核人不得為直屬主管之配偶或三等親等內之血親、姻親");return false;}
	//檢查工作許可
	if(!checkWorkingPeriod(IdNo,start_date,end_date,identity)){return false;}
	
	checkIdentity_Again(OrderNo,bugno,start_date,end_date,PNo,IdNo,identity,payitem,Prank,Ptitle,PayType,Pay_unit,Pay_limit,PayTotal,'add');
}
function modifyUser(){
	//無計畫編號或沒有起訖年月日下拉表未輸入計畫資料
	var OrderNo=document.getElementById('OrderNo').value;
	...
	checkIdentity_Again(OrderNo,bugno,start_date,end_date,PNo,IdNo,identity,payitem,Prank,Ptitle,PayType,Pay_unit,Pay_limit,PayTotal,'modify');
}
function checkRules(OrderNo,bugno,start,end,PNo,IdNo,identity,payitem,Prank,Ptitle,PayTypeStr,Pay_unit,Pay_limit,PayTotal,FromEid,act){
		$.ajax({
		url: 'checkRules.php',
		data:{
			OrderNo:OrderNo,
		...
		success: function(json){
				...
				if(tempmsg!="ok"){
					...
					if(json[i].toString().indexOf("warning")==-1){
						for(j=i-1;j>=0;j--){//去除相同的警語,若之前已有相同警語,就不再顯示
							if(json[i].toString()==json[j].toString()){sameErr=true;}
						}
						if(!sameErr){Msg=Msg+tempmsg+"\n";}
					}else{warning=warning+tempmsg.substring(8)+"\n";}
				}
			}
			json=null;
			//檢查是否需要附件和是否已有相應文件
			...
			Msg = $.trim(Msg);
			uploadMsg = $.trim(uploadMsg);
			warning = $.trim(warning);
			if(Msg!=""){alert(Msg+"\n");}
			else if(uploadMsg!=""){alert("上傳檔案錯誤(PS.只能上傳 pdf/jpg 檔,每個檔案大小上限為2MB)\n"+uploadMsg+"\n");}
			else{
				if(warning!=""){alert("警告:\n"+warning);}
				document.addPT.formAction.value=act;
				
				//alert(Pay_unit+" "+Pay_limit);
				//if(confirm("確定送出??")){
...
function checkWorkingPeriod(IdNo,start,end,identity){
	var isForeign=document.getElementById('isForeign').value;
	var msg="";
	
	if(isForeign=="0"){return true;}
	else{
		//if(document.getElementById('noWorkIDcheck_byRelative').checked){return true;}
		if(document.getElementById('noWorkIDcheck_byPrjLeader').checked){
			var pt=document.getElementById('Ptitle').value;
			//兼任助理和教學助理可選
			if(pt=="3" || pt=="12"){return true;}
			else{
				alert("只有兼任助理和教學助理可選擇「計畫主持人認定屬課程學習範疇之研究助理或教學助理，免申請工作許可」");
				return false;
			}
		}else{
			$.ajax({
				url: 'checkWorkingPeriod.php',
				data:{
					IdNo:IdNo,
					start:start,
					end:end,
...
function checkFileTitle(){
	...
	for(var i = 0; i < filetitle.length; i++){
		tempMsg = $.trim(filetitle[i].value);
		if(tempMsg==""){emptyTitle=1;}
	}
	//檢查未上傳的文件
	filetitle=document.getElementsByName('filetitle[]');
	var uploadfile=document.getElementsByName('upload[]');
	for( i = 0; i < filetitle.length-1; i++){
		tempMsg = $.trim(filetitle[i].value);
		if(tempMsg=="" && uploadfile[i].value!=""){emptyTitle=1;}
	}
	if(emptyTitle==1){
		alert("上傳文件的檔案標題不可空白!!");
		return false;
...
//確認是否需要顯示上傳介面
function ifShowUpload(){
	var Ptitle=document.getElementById('Ptitle').value;//兼任職稱
	//alert(Ptitle+" --");
	if(Ptitle!="4" && Ptitle!="15"){document.getElementById("Div_upload").style.display = "none";}
	else{
		var paytype=get_PayType();
		//alert(paytype);
		if(paytype!=""){
			if(paytype=="hr_pay" || paytype=="award_pay"){document.getElementById("Div_upload").style.display = "none";}
			else{
				...
				$("select[name='filetype[]']").val(filetypeNo);
				//$('.Div_upload option[value='+filetypeNo+']').attr('selected','selected');
			}
		}
	}
	//alert(document.getElementById("Div_upload").style.display);
}
//檢查是否清潔工或臨時工,是的話需上傳檔案,且選對檔案格式
function checkIfNeedUpload(){
	var Ptitle=document.getElementById('Ptitle').value;//兼任職稱
	var tempMsg="";
	if(Ptitle!="4" && Ptitle!="15"){return true;}
	else{
		var paytype=get_PayType();
		//alert(paytype);
		if(paytype=="hr_pay"){return true;}
		else{
			//alert(paytype);
			if(Ptitle=="4" && paytype=="month_pay"){var filetypeNo="9";}
			if(Ptitle=="4" && paytype=="day_pay"){var filetypeNo="10";}
			...
			for(var i = 0; i < filetype.length; i++){
				tempMsg = $.trim(filetype[i].value);
				if(tempMsg==filetypeNo){return true;}
			}
			//檢查未上傳的文件
			filetype=document.getElementsByName('filetype[]');
			var uploadfile=document.getElementsByName('upload[]');
			for( i = 0; i < filetype.length-1; i++){
				//alert(i+"  "+filetype[i].value+"  "+uploadfile[i].value+" a");
				if(filetype[i].value==filetypeNo && uploadfile[i].value!=""){
					return true;
				}
			}
			alert("臨時工或清潔工,若選擇時薪以外支領方式,需檢附文件,並選擇正確的[檔案類型]!!");
			return false;
		}
	}
}
//於送出時再確認一次身份和請核起訖日是否符合
function checkIdentity_Again(OrderNo,bugno,start,end,PNo,IdNo,identity,payitem,Prank,Ptitle,PayTypeStr,Pay_unit,Pay_limit,PayTotal,act){
	$.ajax({
		...
		success: function(json){			
			//原請核追溯超過30天要寫理由,現在好像要改成一天就要寫
			if(parseInt(json['dayDiff'])>=1){
				var reasonCode=document.getElementById('TraceBackReasonCode').value;
				var reason=document.getElementById('TraceBackReason').value;
				reason = $.trim(reason);
				if(reasonCode==""){alert("請核日期有往前追溯，請選擇原因");return false;}
				else if (reasonCode=="8"){
					if(reason==""){alert("請核日期有往前追溯，請填寫原因"+json['dayDiff']);return false;}
				}
			}
			var Msg="";			
			Msg=Msg+json['EmpError'].toString()+"\n"+json['StuError'].toString()+"\n"+json['OutError'].toString()+"\n"+json['UploadError'].toString();
			json=null;	
			Msg = $.trim(Msg);
			if(Msg!=""){
				alert(Msg);
				...
				if(act=="modify"){
					//alert("開始檢核身份囉2");
					checkRules(OrderNo,bugno,start,end,PNo,IdNo,identity,payitem,Prank,Ptitle,PayTypeStr,Pay_unit,Pay_limit,PayTotal,'<?echo $Eid;?>',act);	
				}else{
					//alert("開始檢核身份囉1");
					checkRules(OrderNo,bugno,start,end,PNo,IdNo,identity,payitem,Prank,Ptitle,PayTypeStr,Pay_unit,Pay_limit,PayTotal,'-1',act);
				}
			}
		}
	}); 
