<?php
  ...
  if(strcmp($formAction,"applied")==0){//審核通過
	...
	$db->query($strSQL);
	echo "<script language='javascript'>alert('請核單 ".$OrderNo." 已審核通過');</script>";*/
	...
	//echo $strSQL;
	$result=$db->query($strSQL);
	echo "<script language='javascript'>alert('請核單 ".$OrderNo." 已鎖定');</script>";
	...
	//echo $strSQL;
	$result=$db->query($strSQL);
	echo "<script language='javascript'>alert('請核單 ".$OrderNo." 已退回');</script>";
	...
	if(count($row)>0){
		...
		$result=$db->query($strSQL);
		echo "<script language='javascript'>alert('請核單 ".$OrderNo." 取消已審核狀態');</script>";
	}
	...
	$result=$db->query($strSQL);	
	echo "<script language='javascript'>alert('請核單 ".$OrderNo." 已註銷');</script>";
  }
  
  //處理單子查詢
  //$strSQL="exec sp_getPTApplyList ?";
  ...
  if($_SESSION["power"]=="1"){
	$strSQL="select count(*) as count from ".
		  ...
		  "where (p.SerialNo like '%'+rtrim('".$keytxt."')+'%' or p.BugNo like '%'+rtrim('".$keytxt."')+'%') and p.OrderType='請核' ".
		  "union ".
		  "select p.*,v.name from PT_Outline p ".
		  "left join [PERSONDBOLD].[personnelcommon].[dbo].[vi_Personnel4PartTime] v on v.empno=p.CreateEmp ".
		  "where p.SerialNo in (select SerialNo from PT_Employed where RecordStatus<>'-1' and [Name] like '%'+rtrim('".$keytxt."')+'%') and p.OrderType='請核' ".
		  "union ".
		  "select p.*,v.name from PT_Outline p ".
		  "left join [PERSONDBOLD].[personnelcommon].[dbo].[vi_Personnel4PartTime] v on v.empno=p.CreateEmp ".
	...
	$strSQL="select top ".$page_Show_End." * from ".
		  ...
		  "where p.SerialNo in (select SerialNo from PT_Employed where RecordStatus<>'-1' and (IdCode like '%'+rtrim('".$keytxt."')+'%' ".
		  "or Pid like '%'+rtrim('".$keytxt."')+'%')) ".
		  "and p.OrderType='請核') as a order by SerialNo desc";
  }else{
	$appiedSN=array();
...
<input type="hidden" name="BeginIndex" id="BeginIndex" value="<?echo $page_Show;?>">
  ...
  <input name="keytxt" type="text" value="<?echo $keytxt;?>"><input name="qrybutton" type="button" value="查詢" onclick="javascript:qry();">	

	<table width="1200"  cellspacing="1" cellpadding="4" bgcolor="#9194BF">
		<tr bgcolor="#FF8080">
			<td>&nbsp;</td>
			<td >建立日期</td>
			<td >表單編號</td>
				...
				if($rowindex>=$page_Show_Begin && $rowindex<=$page_Show_End){
					//if(($_SESSION["UserID"]==trim($row['CreateEmp'])) || ($_SESSION["power"]=="1")){
						...
						if($FormStatus=="-1"){//註銷不可列印單子
							...
							echo "<td><a href='https://Receipt-test.nctu.edu.tw/test/phpio_new.php?OrderNo=".trim($row['SerialNo'])."&action=PTform1' target='_blank' >兼任人員請核單</a></td>";
							//echo "<td><a href='form1.php?OrderNo=".trim($row['SerialNo'])."' target='_blank' >兼任人員請核單</a></td>";
...
function unlockForm(OrderNo,reason_id){
	...
	if(confirm("是否退回(解除鎖定)請核單號"+OrderNo)){
		document.Form1.OrderNo.value=OrderNo;
...
function unappliedForm(OrderNo,reason_id){
	...
	if(confirm("是否取消請核單號"+OrderNo+"已審核狀態")){
		document.Form1.OrderNo.value=OrderNo;
