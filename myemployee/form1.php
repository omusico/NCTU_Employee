...
<table border="0" width=981>
	<tr>
		<td colspan="2" align="center"><span style='font-size:16.0pt;font-family:標楷體'>國立交通大學計畫兼任人員及臨時工請核單</span></td>
	</tr>
	<tr>
...
<table class=MsoNormalTable border=1 cellspacing=0 cellpadding=0 width=981
 ...
 <tr style='mso-yfti-irow:2;page-break-inside:avoid;height:28.65pt'>
  <td width=147 colspan=3 style='width:109.9pt;border:solid windowtext 1.0pt;
  border-top:none;mso-border-top-alt:solid windowtext .5pt;mso-border-alt:solid windowtext .5pt;
  padding:0cm 5.4pt 0cm 5.4pt;height:28.65pt'>
  <p class=MsoNormal style='text-align:justify;text-justify:inter-ideograph'><span
  ...
  <td width=581 colspan=6 style='width:177.3pt;border-top:none;border-left:
  none;border-bottom:solid windowtext 1.0pt;border-right:solid windowtext 1.0pt;
  mso-border-top-alt:solid windowtext .5pt;mso-border-left-alt:solid windowtext .5pt;
  mso-border-alt:solid windowtext .5pt;padding:0cm 5.4pt 0cm 5.4pt;height:28.65pt'>
 ...
 <tr style='mso-yfti-irow:4;height:15.25pt'>
  ...
  <?
		...
		if($row['Role']=="E"){echo $row['Title'];}
			...
			echo "校外人士 ";
			if(trim($row['Title'])==""){echo "無學歷資料";}
			else{
				if((int)$row['Title']>3){echo $outer_title[trim($row['Title'])];}
				else{echo $stu_title[trim($row['Title'])];}
			}
		}
  ?></o:p></span></p>
  </td>
  ...
  mso-border-top-alt:solid windowtext .5pt;mso-border-left-alt:solid windowtext .5pt;
  mso-border-alt:solid windowtext .5pt;padding:0cm 5.4pt 0cm 5.4pt;height:15.25pt'>
  <p class=MsoNormal><span lang=EN-US style='font-size:10pt; font-family:標楷體'><o:p><?echo $row['start_y'].addLeadingZeros($row['start_m'],2).addLeadingZeros($row['start_d'],2)."-".$row['end_y'].addLeadingZeros($row['end_m'],2).addLeadingZeros($row['end_d'],2);?></o:p></span></p>
  </td>
  ...
  <?
		...
		if($row['MonthExpense']!=""){$payStr="月薪 ".round($row['MonthExpense'])." 元";}
			...
			if($row['PayType']=="hr_pay"){$payStr="時薪 ".$row['PayPerUnit']." 元，每月上限 ".$row['LimitPerMonth']." 小時";}
			else if($row['PayType']=="day_pay"){$payStr="日薪 ".$row['PayPerUnit']." 元，每月上限 ".$row['LimitPerMonth']." 日";}
			else{$payStr="案件計酬 ".$row['PayPerUnit']." 件，每月上限 ".$row['LimitPerMonth']." 件";}
		}
  ?>
...
<span
  ...
  <?
	echo $row['Memo'];
	//20150416加上延遲理由
	if(trim($row['DelayReason'])!=""){
		echo "<br>追溯請核理由:<br>".trim($row['DelayReason']);
	}
	//20150303依人事室測試回饋要求,加上異動記錄
	$str_mod="select distinct SerialNo from PT_Outline where FormStatus='1' and ".
			 "SerialNo in (select distinct SerialNo from PT_Employed where FirstEid='".$row['Eid']."' and RecordStatus in ('0','-2')) ".
			 "and SerialNo<>'".$OrderNo."'".
			 "order by SerialNo";
	//echo $str_mod;
	$result_mod=$db->query($str_mod);
	$row_mod=$result_mod->fetchAll();
	if(count($row_mod)>0){
		echo "<br>於異動單 ";
		$result_mod=$db->query($str_mod);
		$mod_records= array();
		while($row_mod=$result_mod->fetch()){
			if($row_mod['SerialNo']!=$OrderNo){array_push($mod_records,$row_mod['SerialNo']);}
		}
		echo implode(",",$mod_records);
		echo " 辦理異動";
	}
  ?></o:p></span></p>
  </td>
