<?php
		...
		if($action=="delperson"){
			...
			$sqlDelOuterStatus="update OuterStatus set RecordStatus=-1,UpdateEmpNo='".$empid."' where Eid='".$eid."'";
			
			if($count>0)
				$sqlDelUploadData="update uploaddata set status='-1',UpdateEmp='".$empid."' where peid='".$eid."'";
			else 
			...
			try {
				...
				$queryIn = $db->prepare("update outerstatus set isStudent=:isstudent,isJob=:isjob,schoolName=:school,comName=:com,Education=:edu,studentGrade=:grade,UpdateDate=getdate(),UpdateEmpNo=:empno where eid=:eid");
