...
function checkIdentity($IdCode,$start_y,$start_m,$start_d,$end_y,$end_m,$end_d){
	...
	if($diff<0){$abs_diff=abs($diff);}else{$abs_diff=$diff;}		
	...
	if($CheckDate==0){
		...
		if($getStdDate && $row_getStdDate=$getStdDate->fetch()){
			//若有找到在學資料
			$condition['name']=$row_getStdDate['std_cname'];
			$condition['IdNo']=$row_getStdDate['std_pid'];
...
function checkBugetTypeIFCivil($bugno){
	$pattern="[0-9]{2,3}[CcQq][0-9]{3,5}";//其他計畫
